 ...
 * [반환값] - 화면에 표시될 내용.
 */
export declare type ColumnGroupSummaryCallback = (grid: any, column: DataColumn, footerIndex: number, model: any, value: any) => void;
 ...
 *
 * @remarks
 * {@link CellLayoutItem.direction} 에서 사용된다.
 */
...
export declare interface ColumnObject {
     ...
     *
     * @remarks
     * `gridView`에 등록한 팝업메뉴이름을 지정한다. {@link GridBase.addPopup} 참조
     */
...
export declare interface DataColumn extends ValueColumn {
     ...
     * ```
     */
    lookupData(values: ConfigObject[]): void;
    /**
 ...
 *
 * {@link EditOptions.movable} 과 {@link DisplayOptions.rowMovable} 이 `true` 여야한다.
 *
 * {@link GridBase | 그리드} 에서 옵션으로 설정할 때는 필요한 정보만 넣으면 된다.
 *
 * @example
...
export declare abstract class DataProviderBase {
    ...
    /**
     * 데이터에서 Data 값으로 Parsing하기위한 포맷
     *
     * @defaultValue `null` (기본 포맷 이용)
     *
     * @example
     * ```js
     * ds.datetimeFormat = 'No: Yeah';
     * ```
     */
...
export declare interface DateCellEditor extends TextCellEditor {
     ...
     * 표시시 달력에 선택되는 날짜유형
     *
     * @defaultValue {@link DefaultShowDate}
     */
    defaultShowDate?: DefaultShowDate.NORMAL;
    /**
     * 표시되는 공휴일
     */
    holidays?: Holidays;
...
export declare interface DisplayOptions {
    ...
    /**
     * focus된 셀의 TD에 `rg-focused-cell` class를 추가한다 TR에는 `rg-focused-row`를 추가한다.
     *
     * @defaultValue `false`
     */
    useFocusClass?: boolean;
    /**
     * 그리드 상위 div의 style.display가 변경되는 경우 그리드의 크기를 변경한다.
     *
     * @deafultValue `true`
     */
...
export declare abstract class GridBase {
    ...
    */
     ...
     * {@link GridFootersObj} 의 속성을 가져오거나 설정할 수 있다.
     *
     * @example
     * ```js
     ...
     * {@link HeaderSummariesObj} 의 속성을 가져오거나 설정할 수 있다.
     *
     * @example
     * ```js
     ...
     * ```
     */
    get headerSummaries(): HeaderSummariesObj;
    /**
     * 그리드의 모바일 전용 화면 모델
     *
     * @example
    ...
    /**
     * 그리드 푸터들의 설정 정보들을 {@link GridFooters} 모델로 반환한다.
     *
     * @example
     * ```js
    ...
    /**
     * 그리드 header summary 들의 설정 정보들을 {@link HeaderSummaries} 모델로 반환한다.
     *
     * @example
     * ```js
    ...
    /**
     * 그리드 header summary 들을 {@link HeaderSummaries} 모델로 설정한다.
     *
     * @param value - {@link HeaderSummaries} 모델과 같은 정보. 변경하고자 하는 속성만 전달하면 된다.
     *
     * @example
     *
     * ```js
...
export declare interface GridColumn {
     ...
     * @remarks
     *
     * {@link GridColumn.checkLocation}이 `none`이 아닌경우 체크상태가 화면에 표시된다.
     */
    checked?: boolean;
    /**
     * 컬럼 헤더
 ...
 *
 * @remarks
 * {@link GridBase.getFooter | getFooter()} 을 실행시켰을 경우 0번째 푸터에 대하여 반환한다.
 *
 * {@link GridBase.setFooter | setFooter()} 에서 또는 그리드에서 속성 접근으로 사용 시 원하는 속성만 지정하면 된다.
 *
 * {@link GridFooters} 에서 사용되기도 한다.
 *
 *  @example
 *
 * ```js
 * gridView.setFooter({className: 'custom-footer'});
 * ```
 */
 ...
 *
 * @remarks
 * {@link SubGridFooters} 형태의 오브젝트 또는 {@link GridFooter} 의 배열형태이다.
 *
 * @example
 * ```js
 * // object
 ...
 * ```
 */
export declare type GridFooters = SubGridFooters | GridFooters[];

/**
 * 푸터 영역에 관한 콜렉션 모델 오브젝트
 *
 * @remarks
 * {@link GridBase.footers} 에서 반환되는 객체다.
 *
 * @example
 * ```js
 * gridView.footers.visible = false;
 * ```
 */
export declare interface GridFootersObj {
    /**
     * 표시 여부
     */
    visible: boolean;
 ...
 * @example
 * ```js
 * headerCallback: function(owner, group) {
 * 		if (group.level >= 5) { return false }
 * 		if (group.level === 4) {
 * 			let relation2 = owner.getDataSource().getValue(group.firstItem.dataRow, "relation2");
 * 			return relation2 === "친족"
 ...
 * ```
 */
declare type HeaderCallback = (owner: any, group: GroupItem) => string;

/**
 * Header summary 영역에 관한 콜렉션 모델
 *
 * @remarks
 * {@link SubHeaderSummaries} 형태의 오브젝트 또는 {@link HeaderSummary} 의 배열형태이다.
 *
 * @example
 * ```js
 * // object
 ...
 * ```
 */
export declare type HeaderSummaries = SubHeaderSummaries | HeaderSummary[];
 ...
 * ```
 */
export declare interface HeaderSummariesObj {
    /**
     * 표시 여부
     */
    visible: boolean;
 ...
 *
 * @remarks
 * {@link GridBase.getHeaderSummary | getHeaderSummary()} 을 실행시켰을 경우 0번째 header summary 에 대하여 반환한다.
 *
 * {@link GridBase.setFooter | setHeaderSummary()} 에서 또는 그리드에서 속성 접근으로 사용 시 원하는 속성만 지정하면 된다.
 *
 * {@link HeaderSummaries} 에서 사용되기도 한다.
 *
 * @example
 *
 * ```js
 ...
 */
declare type ProgressCallback = (grid: GridBase, work: string, max: number, position: number) => any;
 ...
 * ```
 */
export declare type RowStyleCallback = (grid: GridBase, item: GridItem, fixed: boolean) => string;
...
export declare interface ShadowDomOptions {
    ...
    /**
     * shadow그리드에서 편집버튼을 클릭했을때 실행되는 콜백
     *
     * @remarks
...
/**
 * 푸터 영역에 관한 콜렉션 설정 모델 중 하나
 *
 * @remarks
 * {@link GridFooters} 의 형태 중 하나다.
 *
 * @example
 * ```js
 * // object
 ...
 * ```
 */
declare interface SubGridFooters {
    /**
     * 표시 여부
     */
    visible?: boolean;
...
/**
 * Header Summary 영역에 관한 콜렉션 설정 모델 중 하나
 *
 * @remarks
 * {@link HeaderSummaries} 의 형태 중 하나이다.
 *
 * @example
 * ```js
 * // object
 ...
 * ```
 */
declare interface SubHeaderSummaries {
    /**
     * 표시 여부
     */
    visible?: boolean;
